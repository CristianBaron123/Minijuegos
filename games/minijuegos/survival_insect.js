// ============================================
// MINIJUEGO: SURVIVAL INSECT - Evita al insecto que te persigue
// Al tocarlo apareces en y=-360. El último en pie gana.
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[SURVIVAL_INSECT] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[SURVIVAL_INSECT] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('⚠️ No hay suficientes jugadores para SURVIVAL INSECT', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            '\n📋 INSTRUCCIONES:\n' +
            '🐛 ¡Evita al insecto que te persigue!\n' +
...
function checkPlayers(room) {
    ...
    for (var i = 0; i < gameState.players.length; i++) {
        ...
        if (py <= config.eliminationY) {
            ...
            room.sendAnnouncement('❌ ' + p.name + ' fue atrapado por el insecto! (' + remaining + ' restantes)', null, 0xFF6600);
        } else {
            alivePlayers.push(p);
        }
    }
...
function declareWinner(room, winner) {
    ...
    room.sendAnnouncement('\n🏆 ¡' + winner.name.toUpperCase() + ' HA GANADO SURVIVAL INSECT! 🏆\n', null, 0xFFD700, 'bold', 2);
