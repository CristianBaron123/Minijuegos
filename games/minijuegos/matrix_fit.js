// ============================================
// MINIJUEGO: MATRIX FIT - Esquiva obstaculos en la Matrix
// Eliminado si sube demasiado (y <= -270)
// ============================================
...
var config = {
    upperLimit: -270  // si y <= -270, eliminado (subio demasiado alto)
};

function start(room, onGameEnd) {
    if (!mapData) { console.error('[MATRIX_FIT] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[MATRIX_FIT] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('No hay suficientes jugadores para MATRIX FIT', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            '\n🟩 MATRIX FIT\n' +
            '💊 Esquiva los obstaculos de la Matrix!\n' +
            '💀 Si un obstaculo te empuja fuera del area, quedas eliminado\n' +
...
function checkPlayers(room) {
    ...
    for (var i = 0; i < gameState.players.length; i++) {
        ...
        if (pos.y <= config.upperLimit) {
            eliminatePlayer(room, p, 'fue empujado fuera');
        }
    }
}
...
function checkWinner(room) {
    ...
    if (alive.length === 1) {
        ...
        room.sendAnnouncement('\n🏆 ' + alive[0].name.toUpperCase() + ' HA GANADO MATRIX FIT! 🏆', null, 0xFFD700, 'bold', 2);
