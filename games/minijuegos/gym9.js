// ============================================
// MINIJUEGO: GYM 9 - Evita los bordes, quedate en el cuadro
// Eliminado si sale del cuadro (-794,-300) a (794,300)
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[GYM9] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[GYM9] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('No hay suficientes jugadores para GYM 9', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            '\n🏋️ GYM 9\n' +
            '🏃 Evita los bordes y quedate dentro del cuadro!\n' +
            '📦 El cuadro se reduce con el tiempo\n' +
...
function checkWinner(room) {
    ...
    if (alive.length === 1) {
        ...
        room.sendAnnouncement('\n🏆 ' + alive[0].name.toUpperCase() + ' HA GANADO GYM 9! 🏆', null, 0xFFD700, 'bold', 2);
