// ============================================
// MINIJUEGO: PLUM - Goles inversos con eliminacion
// La pelota se repele al patearla (kickStrength negativo).
// Anota golpeando el balon desde el lado contrario.
// Eliminacion por rondas: equipo perdedor sale.
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[PLUM] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;
    gameState.stopRequested = false;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[PLUM] Error cargando mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('⚠️ No hay suficientes jugadores para PLUM', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    room.sendAnnouncement('🟣 PLUM\n👥 Jugadores: ' + players.length, null, 0x9B59B6, 'bold', 2);
    ...
    if (gameState.firstRound) {
        ...
        setTimeout(function() {
            ...
            room.sendAnnouncement('\n📋 INSTRUCCIONES:\n' +
                '🟣 El balon se REPELE al patearlo!\n' +
                '⚽ Golpea desde tu lado para empujar al arco CONTRARIO\n' +
                '🔴 vs 🔵 - Primero a ' + config.goalsToWin + ' goles gana la ronda\n' +
        ...
        while (!gameState.stopRequested) {
            ...
            if (gameState.players.length === 2) {
                ...
                if (result && result.winners && result.winners.length > 0) {
                    ...
                    if (p) {
                        room.sendAnnouncement('\n🏆 ' + p.name.toUpperCase() + ' HA GANADO PLUM! 🏆', null, 0xFFD700, 'bold', 2);
