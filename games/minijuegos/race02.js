// ============================================
// MINIJUEGO: RACE 02 - Carrera de izquierda a derecha
// El primero en cruzar la meta gana
// Meta en x=11440, entre y=-70 y y=-40
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[RACE02] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;
    gameState.winnerDeclared = false;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[RACE02] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('No hay suficientes jugadores para RACE 02', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            '\n🏁 RACE 02\n' +
            '🏃 Corre hacia la derecha lo mas rapido posible!\n' +
            '🏆 El primero en cruzar la meta GANA!\n' +
            '⏳ Comienza en 5s...',
...
function checkPlayers(room) {
    ...
    for (var i = 0; i < gameState.players.length; i++) {
        ...
        // Cruza meta si x >= lineX y y entre lineMinY y lineMaxY
        if (pos.x >= config.lineX && pos.y >= config.lineMinY && pos.y <= config.lineMaxY) {
...
function declareWinner(room, winner) {
    ...
    room.sendAnnouncement(
        '\n🏆 ' + winner.name.toUpperCase() + ' HA GANADO RACE 02! 🏆',
