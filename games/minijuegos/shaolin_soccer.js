// ============================================
// MINIJUEGO: SHAOLIN SOCCER - Torneo de goles
// Parecido a ping pong: golpea el balon
// Primero a 3 goles gana. Torneo eliminacion.
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[SHAOLIN_SOCCER] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;
    gameState.stopRequested = false;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[SHAOLIN_SOCCER] Error cargando mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('⚠️ No hay suficientes jugadores para SHAOLIN SOCCER', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    room.sendAnnouncement('🥋 SHAOLIN SOCCER! Primero a ' + config.goalsToWin + ' goles gana.', null, 0xFF4500, 'bold', 2);
    ...
    if (gameState.firstRound) {
        ...
        setTimeout(function() {
            ...
            room.sendAnnouncement('\n📋 INSTRUCCIONES:\n' +
                '🥋 Golpea el balon hasta que toque el piso del rival!\n' +
                '🏓 Es como ping pong: pegale arriba y caera del otro lado\n' +
        ...
        while (!gameState.stopRequested) {
            ...
            if (gameState.players.length === 2) {
                ...
                if (result && result.winners && result.winners.length > 0) {
                    ...
                    if (p) {
                        room.sendAnnouncement('\n🏆 ' + p.name.toUpperCase() + ' HA GANADO SHAOLIN SOCCER! 🏆', null, 0xFFD700, 'bold', 2);
                        setTimeout(function() {
