// ============================================
// MINIJUEGO: BONK WINTER by Serefli Seref
// Rebota golpeando las bolas de nieve y municos de nieve
// El ultimo en pie gana. Eliminado si cae a y <= -420
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[BONK_WINTER] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[BONK_WINTER] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < config.minPlayers) {
        room.sendAnnouncement('⚠️ No hay suficientes jugadores para BONK WINTER', null, 0xFF6600);
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            '\n📋 INSTRUCCIONES:\n' +
            '❄️ Estas en la nieve!\n' +
            '⛄ Golpea las bolas de nieve y municos de nieve para rebotar\n' +
        ...
        setTimeout(function() {
            ...
            room.sendAnnouncement('🟢 COMIENZA BONK WINTER!', null, 0x00FF00, 'bold', 2);
...
function checkPlayers(room) {
    ...
    // Anunciar eliminaciones
    if (alivePlayers.length < gameState._lastAliveCount) {
        var newElim = gameState.players.filter(function(p) {
            return gameState.eliminated.indexOf(p.id) !== -1 && !gameState._announcedElim[p.id];
        });
        for (var ei = 0; ei < newElim.length; ei++) {
            gameState._announcedElim[newElim[ei].id] = true;
            room.sendAnnouncement('❌ ' + newElim[ei].name + ' eliminado! (' + alivePlayers.length + ' restantes)', null, 0xFF6600);
        }
    }
...
function declareWinner(room, winner) {
    ...
    room.sendAnnouncement(
        '\n🏆 ' + winner.name.toUpperCase() + ' HA GANADO BONK WINTER! 🏆',
