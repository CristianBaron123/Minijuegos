// ============================================
// MINIJUEGO: COVID-19 by Namajunas
// Esquiva el virus COVID, ultimo en pie gana
// El virus te lanza a la linea negra de arriba = eliminado
// ============================================
...
var config = {
    ...
    bounds: { minX: -790, maxX: 790, minY: -310, maxY: 160 },
    eliminateY: -80
};

function start(room, onGameEnd) {
    if (!mapData) { console.error('[COVID_NAMAJUNAS] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    ...
    try { room.setCustomStadium(mapData); } catch(e) { console.error('[COVID_NAMAJUNAS] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < config.minPlayers) {
        room.sendAnnouncement('⚠️ No hay suficientes jugadores para COVID-19', null, 0xFF6600);
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            ...
            '💀 Si un virus te toca, te lanza a la linea negra = ELIMINADO\n' +
        ...
        setTimeout(function() {
            ...
            gameState.prevPositions = {};
            room.sendAnnouncement('🟢 ¡COMIENZA COVID-19 SURVIVAL!', null, 0x00FF00, 'bold', 2);
...
function declareWinner(room, winner, onGameEnd) {
    ...
    room.sendAnnouncement(
        '\n🏆 ¡' + winner.name.toUpperCase() + ' SOBREVIVIO AL COVID-19! 🏆\n🦠 Ultimo en pie!',
...
function repositionSpawns(room) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        var sy = Math.round(safeY + cellH * row);
