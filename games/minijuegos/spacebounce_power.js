// ============================================
// MINIJUEGO: SPACEBOUNCE POWER - Rapido y con porteros rapidos
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[SPACEBOUNCE_POWER] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;
    gameState.stopRequested = false;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[SPACEBOUNCE_POWER] Error cargando mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0 && p.team !== 0; });
    if (players.length < 2) { room.sendAnnouncement('⚠️ No hay suficientes jugadores para SPACEBOUNCE POWER', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    room.sendAnnouncement('🚀 SPACEBOUNCE POWER - Velocidad maxima! Primero a ' + config.goalsToWin + ' goles gana.', null, 0x00BFFF, 'bold', 2);
    ...
    if (gameState.firstRound) {
        ...
        setTimeout(function() {
            ...
            room.sendAnnouncement('\n📋 INSTRUCCIONES:\n' +
                '🚀 Vas super rapido y los porteros se mueven rapido!\n' +
    ...
    (async function loop() {
        while (!gameState.stopRequested) {
            ...
            if (gameState.players.length === 2) {
                ...
                // 50% chance de juego especial 1v1
                var _finaleGames = [];
                ...
                if (result && result.winners && result.winners.length > 0) {
                    ...
                    if (p) {
                        room.sendAnnouncement('\n🏆 ' + p.name.toUpperCase() + ' HA GANADO SPACEBOUNCE POWER! 🏆', null, 0xFFD700, 'bold', 2);
                        setTimeout(function() {
