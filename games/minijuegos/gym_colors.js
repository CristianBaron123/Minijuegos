// ============================================
// MINIJUEGO: GYM 5.3 COLORS - Entra a los circulos de tu color
// Eliminado si sale del cuadro (-425,-205) a (425,205)
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[GYM_COLORS] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[GYM_COLORS] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('No hay suficientes jugadores para GYM COLORS', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            '\n🎨 GYM 5.3 COLORS\n' +
            '🔴🔵 Entra a los circulos de TU color o al VERDE!\n' +
            '🟣 Usa las lineas moradas para rebotar a tus rivales\n' +
            '💀 Si sales del area, quedas eliminado\n' +
            '🏆 El ultimo en pie GANA!\n' +
            '⏳ Comienza en 5s...',
            null, 0xFF00FF, 'bold', 2
        );
...
function checkWinner(room) {
    ...
    if (alive.length === 1) {
        ...
        room.sendAnnouncement('\n🏆 ' + alive[0].name.toUpperCase() + ' HA GANADO GYM COLORS! 🏆', null, 0xFFD700, 'bold', 2);
