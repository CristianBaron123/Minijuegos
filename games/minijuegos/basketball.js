// ============================================
// MINIJUEGO: BASKETBALL By GLH
// Primero a 2 canastas gana. Torneo eliminacion.
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[BASKETBALL] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;
    gameState.stopRequested = false;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[BASKETBALL] Error cargando mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('⚠️ No hay suficientes jugadores para BASKETBALL', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    room.sendAnnouncement('🏀 BASKETBALL! Primero a ' + config.goalsToWin + ' canastas gana.', null, 0xFF6600, 'bold', 2);
    ...
    if (gameState.firstRound) {
        ...
        setTimeout(function() {
            ...
            room.sendAnnouncement('\n📋 INSTRUCCIONES:\n' +
                '🏀 Mete la pelota en la canasta del rival!\n' +
                '⚽ Primero a ' + config.goalsToWin + ' canastas gana la ronda\n' +
                '🔄 Eliminacion por rondas hasta el 1vs1\n' +
                '⏳ Comienza en 5s...'
    ...
    (async function loop() {
        while (!gameState.stopRequested) {
            ...
            if (gameState.players.length === 3) {
                ...
                room.sendAnnouncement('ℹ️ Se movio a un jugador a espectador para equilibrar.', null, 0xFFFF00);
            }
            ...
            if (gameState.players.length === 2) {
                ...
                if (result && result.winners && result.winners.length > 0) {
                    ...
                    if (p) {
                        room.sendAnnouncement('\n🏆 ' + p.name.toUpperCase() + ' HA GANADO BASKETBALL! 🏆', null, 0xFFD700, 'bold', 2);
                        setTimeout(function() {
...
function playMatch(room, playerIds, goalsToWin, timeMs) {
    return new Promise(function(resolve) {
        ...
        room.onTeamGoal = function(team) {
            ...
            if (team === 1 || team === 2) {
                scores[team]++;
                room.sendAnnouncement('🏀 Canasta del equipo ' + (team === 1 ? '🔴' : '🔵') + ' — ' + scores[1] + ' : ' + scores[2], null, 0x00FF00);
        ...
        var to = gameState.matchTimeout = setTimeout(function() {
            ...
            if (scores[1] === scores[2]) {
                room.sendAnnouncement('\n⚡ Empate. Muerte Subita: SIGUIENTE CANASTA GANA!', null, 0xFFFF00);
                stopped = false;
                room.onTeamGoal = function(team) {
                    if (stopped) return;
                    stopped = true;
                    room.sendAnnouncement('🏀 CANASTA! Equipo ' + (team === 1 ? '🔴' : '🔵') + ' GANA en muerte subita!', null, 0xFFD700, 'bold', 2);
                    cleanupAndResolve(team);
                };
                return;
            }
        ...
        function cleanupAndResolve(team) {
            ...
            for (var i = 0; i < playerIds.length; i++) {
                ...
                if (teamOf === team) winners.push(playerIds[i]); else losers.push(playerIds[i]);
            }
