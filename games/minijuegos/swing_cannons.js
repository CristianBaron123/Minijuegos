// ============================================
// MINIJUEGO: SWING CANNONS - Usa los cañones para no salir del área
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[SWING_CANNONS] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[SWING_CANNONS] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0; });
    if (players.length < 2) { room.sendAnnouncement('⚠️ No hay suficientes jugadores para SWING CANNONS', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    setTimeout(function() {
        ...
        room.sendAnnouncement(
            '\n📋 INSTRUCCIONES:\n' +
            '💥 ¡Usa los cañones para impulsarte!\n' +
            '⚠️ Si sales del área, quedas eliminado\n' +
            '🏆 ¡El último jugador en pie gana!\n\n' +
...
function declareWinner(room, winner) {
    ...
    room.sendAnnouncement('\n🏆 ¡' + winner.name.toUpperCase() + ' HA GANADO SWING CANNONS! 🏆\n', null, 0xFFD700, 'bold', 2);
