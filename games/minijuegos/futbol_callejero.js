// ============================================
// MINIJUEGO: FUTBOL CALLEJERO A LAS PINAS
// Partido de goles con eliminacion por rondas, primero a 3 goles
// ============================================
...
function start(room, onGameEnd) {
    if (!mapData) { console.error('[FUTBOL_CALLEJERO] mapData no inyectado.'); if (onGameEnd) onGameEnd(null); return; }
    gameState.callback = onGameEnd || null;
    gameState.stopRequested = false;

    try { room.setCustomStadium(mapData); } catch(e) { console.error('[FUTBOL_CALLEJERO] Error mapa', e.message); if (onGameEnd) onGameEnd(null); return; }

    var players = room.getPlayerList().filter(function(p) { return p.id !== 0 && p.team !== 0; });
    if (players.length < 2) { room.sendAnnouncement('No hay suficientes jugadores para FUTBOL CALLEJERO', null, 0xFF6600); if (onGameEnd) onGameEnd(null); return; }
    ...
    room.sendAnnouncement('⚽ FUTBOL CALLEJERO A LAS PINAS - Primero a ' + config.goalsToWin + ' goles gana cada ronda.', null, 0xFF4500, 'bold', 2);
...
function runTournament(room) {
    (async function loop() {
        while (!gameState.stopRequested) {
            ...
            try { room.stopGame(); } catch(e) {}
            for (var li = 0; li < result.losers.length; li++) {
                var id = result.losers[li];
            ...
            if (gameState.players.length === 1 && gameState.spectatorPool.length > 0) {
                var rndIdx3 = Math.floor(Math.random() * gameState.spectatorPool.length);
                var picked3 = gameState.spectatorPool.splice(rndIdx3, 1)[0];
                gameState.players.push(picked3);
                try { room.setPlayerTeam(picked3, 1); } catch(e) {}
            }
            if (gameState.players.length <= 1) {
                if (gameState.players.length === 1) {
    ...
    return new Promise(function(resolve) {
        ...
        var to = gameState.matchTimeout = setTimeout(function() {
            ...
            if (scores[1] === scores[2]) {
                room.sendAnnouncement('\n⚡ Empate. Muerte Subita: SIGUIENTE GOL GANA!', null, 0xFFFF00);
                stopped = false;
                room.onTeamGoal = function(team) {
                    if (stopped) return;
                    stopped = true;
        ...
        function cleanupAndResolve(team) {
            ...
            var winners = [];
            var losers = [];
            var h = Math.floor(playerIds.length / 2);
            for (var i = 0; i < playerIds.length; i++) {
                var id = playerIds[i];
